function SkinConfig(_rel_x = 0, _rel_y = 0, _w = 400, _h = 225): Control(_rel_x, _rel_y, _w, _h) constructor {
    rel_x = round((room_width - _w) / 2);
    rel_y = round((room_height - _h) / 2);
    ...
    btn_apply.rel_x = list_w + 8;
    btn_apply.rel_y = _h - 8 - btn_apply.h;
    ...
    preview.on_draw = function() {
        ...
        if (bg_type == BG_TYPE_STRETCH)
            draw_sprite_stretched(bg_background_preview, 0, x + list_w, y + 1, w - list_w - 1, h - 2);
        else
            draw_sprite_tiled_area(bg_background_preview, 0, 0, 0, x + list_w, y + 1, x + w - 2, y + h - 2);
